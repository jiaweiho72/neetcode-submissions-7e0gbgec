class Solution:
    def combinationSum2(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        9 Aug 2026
        - contains duplicates
        - Same as I but now candidates can use at most once
        """

        n = len(candidates)
        candidates.sort() # sort so that you can side by side compare and get ignore duplicates
        result = []

        def dfs(i, cur_nums, cur_sum):
            if cur_sum == target:
            ...
            for j in range(i, n):
                # handle duplicate
                if j > i and candidates[j - 1] == candidates[j]: # deciding whether to skip current element
                    continue

                cur_nums.append(candidates[j]) # choose
                dfs(j + 1, cur_nums, cur_sum + candidates[j])
                cur_nums.pop() # don't choose
