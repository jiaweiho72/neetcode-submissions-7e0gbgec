class MyQueue:
    ...
    and it is inserted n times
    - the time complexity is looking at PER OPERATION
        - so per operation some best case operations don't need to shift anything, some worst case operation
        shift up to n elements. So ammortized looking at the best and the worst being total O(n) divided
        by the number of commands which is O(1)
        - each operation is between O(1) to O(n) but it won't be all O(n)
    Space: 2 n stack
    """
