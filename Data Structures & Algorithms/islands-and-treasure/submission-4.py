class Solution:
    def islandsAndTreasure(self, grid: List[List[int]]) -> None:
        ...
        Time:
        - Get treasures O(V)
        - BFS O(V + E) but since matrix so E ~ V so O(V)
        ...
        def bfs(): # no input as the start is not a point, but every point in treasure_list
            ...
            while queue:
                ...
                for i in range(level_size):
                    ...
                    may lead to additional calls but the answer is still correct as once it hits land, it still properly check  visited"""
                    # if cur == 0:
                    #     continue
                    if cur == 2147483647:
