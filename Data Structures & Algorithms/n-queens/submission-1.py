class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
        ...
        m is no of rows, n no of cols
        Time: O(n!)
        - recursion tree n^n
            - depth/height = rows
            - each branch factor = col
            col ^ rows
        - but once a col in col/diag set, it won't be processed again
            so each dfs call, the branch factor decrease (n) * (n-1) * (n-2) ... = n!
        
        Space O(n^2 * no_of_solutions)
        sets -> n
        each board n^2
        """
