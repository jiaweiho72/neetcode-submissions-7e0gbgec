class Solution:
    def openLock(self, deadends: List[str], target: str) -> int:
        ...
        Let n be no of possible digit 10C1 ^4 (not permutation as this question can repeat same digit)
        m be no of dead ends
        Time: O(n)
        - n times roughly processing the queue (more as duplicate visited items)
            - m in inside these n times
