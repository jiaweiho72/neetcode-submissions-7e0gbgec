class Solution:
    def subsets(self, nums: List[int]) -> List[List[int]]:
        """
        9 Sep
        - return all possible subsets - must not contain duplicates
            - nums are unique integers so no need

        backtracking
        - dfs(i) where i is which part of nums
        - keep track of global result list
        ...
        def dfs(i, cur_subset):
            # save every combination -> no base case
        ...
        result = []
        def dfs(i, cur_path):
            # no base case - every path copy down
            result.append(cur_path.copy())

            for j in range(i, n):
                cur_path.append(nums[j]) # choose
                dfs(j + 1, cur_path)
